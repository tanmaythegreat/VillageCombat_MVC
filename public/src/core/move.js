...
export function moveUpdate(grid_x,grid_y){
    if (!moving) return
    if (grid_x===prevPosX && grid_y===prevPosY) return;
...
export function selectToMove(){
    moving = true;
    prevPosX = _placed_building.grid_x
    prevPosY = _placed_building.grid_y
    inicial_posY = _placed_building.grid_y
    inicial_posX = _placed_building.grid_x
...
function excapeMove(){
    if (!moving)return
    moveUpdate(inicial_posX,inicial_posY)
    moving = false
